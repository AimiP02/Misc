...
Address_t FindPattern(const std::string_view szModuleName,
  ...
  if (pModule == nullptr)
    throw std::runtime_error(
        std::vformat("Failed to get base address for \"{}\" module.",
                     std::make_format_args(szModuleName)));

  const IMAGE_DOS_HEADER *pDosHeader = pModule.Cast<const IMAGE_DOS_HEADER *>();
  const IMAGE_NT_HEADERS *pNtHeaders =
      pModule.Offset<const IMAGE_NT_HEADERS *>(pDosHeader->e_lfanew);

  return FindPattern(pModule, pNtHeaders->OptionalHeader.SizeOfImage,
                     szPattern);
}
...
Address_t FindPattern(const Address_t pRegionStart,
  ...
  const std::vector<std::optional<std::uint8_t>> vecBytes =
      PatternToBytes(szPattern);

  for (std::uintptr_t uIndex = 0, uSize = unRegionSize - vecBytes.size();
       uIndex < uSize; ++uIndex) {
    bool bByteFound = true;

    for (std::uintptr_t uByteIndex = 0; uByteIndex < vecBytes.size();
         ++uByteIndex) {
      if (vecBytes[uByteIndex].has_value() &&
          pBytes[uIndex + uByteIndex] != vecBytes[uByteIndex].value()) {
        bByteFound = false;
        break;
      }
    }
...
Address_t GetExportAddress(const Address_t pModuleBase,
  ...
  const IMAGE_DOS_HEADER *pDosHeader =
      pModuleBase.Cast<const IMAGE_DOS_HEADER *>();
  const IMAGE_NT_HEADERS *pNtHeaders =
      pModuleBase.Offset<const IMAGE_NT_HEADERS *>(pDosHeader->e_lfanew);
  const IMAGE_OPTIONAL_HEADER *pOptionalHeader = &pNtHeaders->OptionalHeader;

  const std::uint32_t uExportSize =
      pOptionalHeader->DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT].Size;
  const std::uint32_t uExportAddress =
  ...
  const IMAGE_EXPORT_DIRECTORY *pExportDirectory =
      pModuleBase.Offset<const IMAGE_EXPORT_DIRECTORY *>(uExportAddress);
  const std::uint32_t *pNamesRVA = pModuleBase.Offset<const std::uint32_t *>(
      pExportDirectory->AddressOfNames);
  const std::uint32_t *pFunctionsRVA =
      pModuleBase.Offset<const std::uint32_t *>(
          pExportDirectory->AddressOfFunctions);
  const std::uint16_t *pNameOrdinals =
      pModuleBase.Offset<const std::uint16_t *>(
  ...
  while (uRight != uLeft) {
    ...
    const int iResult = szProcedureName.compare(
        pModuleBase.Offset<const char *>(pNamesRVA[uMiddle]));
...
PatternToBytes(const std::string_view szPattern) {
  ...
  while (it < itEnd) {
    if (*it == '?') {
      ...
      std::uint8_t uByte = static_cast<std::uint8_t>(
          ((*it >= 'A' ? (((*it - 'A') & (~('a' ^ 'A'))) + 10)
                       : (*it <= '9' ? *it - '0' : 0x0)) |
           0xF0)
          << 4);
...
std::string BytesToPattern(const std::uint8_t *pBytes,
                           const std::size_t uSize) {
  ...
  for (std::size_t nIndex = 0U, nByteIndex = 0U; nIndex < nHexLength;
       nByteIndex++, nIndex += 3U) {
